//! Native macOS deletion engine. Compiles only on `cfg(target_os =
//! "macos")` targets.
//!
//! Uses Darwin/POSIX-native primitives throughout (`opendir`/`readdir`,
//! `openat`/`unlinkat`/`fstatat`), not a POSIX layer borrowed from the
//! Linux engine translated verbatim -- see `docs/adr/0001-workspace-architecture.md`.
//! `--kill-locks` uses `lsof` for lock-holder identification (see
//! `lock.rs` for why); `--close-remote-locks` is always reported
//! unsupported, per the product brief's requirement that this remain a
//! Windows-server-specific capability.
...
impl PlatformEngine for MacEngine {
    ...
    fn enumerate(
        ...
        stream_tree(root, &dirlist::MacDirLister, sink, cancel)
    }
