//! Delete and remediation operations for the macOS engine.
//!
...
//! `unlinkat`/`fstatat` relative to that descriptor rather than operating
//! on the full path string a second time. See
//! `docs/adr/0006-posix-toctou.md` for exactly what this does and does not
//! protect against, and why a full fd-chained walk (eliminating the
//! ancestor-chain race entirely) is documented as follow-up hardening
...
/// modest compared to the Windows engine's ownership/ACL remediation --
/// Unix deletion authority is governed by the *parent directory's* write
/// permission, not the object's own permission bits, so there is
/// meaningfully less for "remediation" to do here; see
